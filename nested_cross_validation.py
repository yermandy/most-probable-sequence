...
for split in [0, 1, 2, 3, 4]:
    ...
    with open(
        f"{tst_files_root}/tst/split_{split}/shuffled/whole_file/y.pickle", "rb"
    ) as f:
        y_true = pickle.load(f)

    with open(
        f"{tst_files_root}/tst/split_{split}/shuffled/whole_file/features.pickle", "rb"
    ) as f:
    ...
    for inner_split in range(5):
        ...
        for run in runs:
            if "cross_validation_fold" in run.config:
                ...
                rvces_run = evaluate(
                    features, y_true, weights_root, run_name, predictions=predictions
                )
